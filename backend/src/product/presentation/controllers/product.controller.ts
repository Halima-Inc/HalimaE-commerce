import {
    Controller,
    Get,
    Post,
    Body,
    Patch,
    Param,
    Delete,
    UseGuards,
    Query,
    HttpCode,
    HttpStatus,
} from '@nestjs/common';
import {
    ApiTags,
    ApiOperation,
    ApiBearerAuth,
    ApiBody,
    ApiExtraModels,
} from '@nestjs/swagger';
import {
    ApiStandardResponse,
    ApiStandardErrorResponse,
    ApiStandardNoContentResponse,
} from '../../../common/swagger/api-response.decorator';
import {
    CreateProductDto,
    FilterProductDto,
    FinalizeProductImageUploadDto,
    ProductImageDto,
    ProductImageUploadUrlResponseDto,
    ProductVariantDto,
    ResponseProductDto,
    ResponseVariantDto,
    UpdateProductDto,
    UpdateVariantDto,
    CreateProductImageUploadUrlDto,
} from '../dto';
import {
    JwtAccessTokenGuard,
    RequiredRoles,
    RolesGuard,
} from '../../../auth/presentation';
import { ResponseProductFilteredDto } from '../dto/product/response-product-filtered.dto';
import { VariantPriceDto } from '../dto/variant/variant-price.dto';
import { VariantInventoryDto } from '../dto/variant/variant-inventory.dto';
import {
    CreateProductImageUploadUrlCommand,
    GetProductByIdHandler,
    GetProductImagesHandler,
    GetProductImagesQuery,
    GetProductVariantsHandler,
    GetProductVariantsQuery,
    ListProductsHandler,
    CreateProductHandler,
    CreateProductImageUploadUrlHandler,
    UpdateProductHandler,
    UpdateProductVariantHandler,
    DeleteProductHandler,
    DeleteProductVariantHandler,
    DeleteProductImageHandler,
    FinalizeProductImageUploadHandler,
    ReplaceProductImageHandler,
    CreateProductVariantHandler,
    CreateProductCommand,
    CreateProductVariantCommand,
    UpdateProductCommand,
    UpdateProductVariantCommand,
    DeleteProductCommand,
    DeleteProductVariantCommand,
    DeleteProductImageCommand,
    FinalizeProductImageUploadCommand,
    ReplaceProductImageCommand,
    GetProductByIdQuery,
    ListProductsQuery,
} from '../../application';

@ApiTags('products')
@ApiExtraModels(
    CreateProductDto,
    UpdateProductDto,
    ResponseProductDto,
    ResponseProductFilteredDto,
    FilterProductDto,
    ProductVariantDto,
    UpdateVariantDto,
    ResponseVariantDto,
    ProductImageDto,
    VariantPriceDto,
    VariantInventoryDto,
    CreateProductImageUploadUrlDto,
    FinalizeProductImageUploadDto,
    ProductImageUploadUrlResponseDto,
)
@Controller('products')
export class ProductController {
    constructor(
        private readonly listProductsHandler: ListProductsHandler,
        private readonly getProductByIdHandler: GetProductByIdHandler,
        private readonly createProductHandler: CreateProductHandler,
        private readonly updateProductHandler: UpdateProductHandler,
        private readonly deleteProductHandler: DeleteProductHandler,
        private readonly getProductVariantsHandler: GetProductVariantsHandler,
        private readonly createProductVariantHandler: CreateProductVariantHandler,
        private readonly updateProductVariantHandler: UpdateProductVariantHandler,
        private readonly deleteProductVariantHandler: DeleteProductVariantHandler,
        private readonly getProductImagesHandler: GetProductImagesHandler,
        private readonly createProductImageUploadUrlHandler: CreateProductImageUploadUrlHandler,
        private readonly finalizeProductImageUploadHandler: FinalizeProductImageUploadHandler,
        private readonly replaceProductImageHandler: ReplaceProductImageHandler,
        private readonly deleteProductImageHandler: DeleteProductImageHandler,
    ) {}

    @Get()
    @ApiOperation({
        summary: 'Get all products with filters',
        description:
            'Retrieve a paginated list of products with optional filters for name, category, status, and price range',
    })
    @ApiStandardResponse(
        ResponseProductFilteredDto,
        'Products retrieved successfully',
    )
    @ApiStandardErrorResponse(
        400,
        'Invalid filter parameters',
        'Invalid filter parameters provided',
    )
    async getProducts(@Query() filters: FilterProductDto) {
        return this.listProductsHandler.execute(new ListProductsQuery(filters));
    }

    @Get(':id')
    @ApiOperation({
        summary: 'Get product by ID',
        description: 'Retrieve detailed information about a specific product',
    })
    @ApiStandardResponse(ResponseProductDto, 'Product retrieved successfully')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    async getProductById(@Param('id') id: string): Promise<ResponseProductDto> {
        return this.getProductByIdHandler.execute(new GetProductByIdQuery(id));
    }

    @Get(':id/variants')
    @ApiOperation({
        summary: 'Get product variants',
        description:
            'Retrieve all variants (sizes, colors, etc.) for a specific product',
    })
    @ApiStandardResponse(ResponseVariantDto, 'Variants retrieved successfully')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    async getProductVariants(
        @Param('id') id: string,
    ): Promise<ResponseVariantDto[]> {
        return this.getProductVariantsHandler.execute(
            new GetProductVariantsQuery(id),
        );
    }

    @Get(':id/images')
    @ApiOperation({
        summary: 'Get product images',
        description: 'Retrieve all images for a specific product',
    })
    @ApiStandardResponse(ProductImageDto, 'Images retrieved successfully')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    async getProductImages(@Param('id') id: string) {
        return this.getProductImagesHandler.execute(
            new GetProductImagesQuery(id),
        );
    }

    // 1. create prduct with its variants routes
    @Post()
    @ApiOperation({
        summary: 'Create new product',
        description:
            'Create a new product with its variants. Requires admin or employee role.',
    })
    @ApiBearerAuth()
    @ApiStandardResponse(
        ResponseProductDto,
        'Product created successfully',
        201,
    )
    @ApiStandardErrorResponse(
        400,
        'Invalid product data',
        'Validation failed for product creation',
    )
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Category not found',
        'The specified category does not exist',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.CREATED)
    async createProduct(
        @Body() productDto: CreateProductDto,
    ): Promise<ResponseProductDto | null> {
        return this.createProductHandler.execute(
            new CreateProductCommand(productDto),
        );
    }

    @Post(':id/images/upload-url')
    @ApiOperation({
        summary: 'Create signed image upload URL',
        description:
            'Create a pre-signed URL for direct S3 image upload. The returned key must be sent to finalize endpoint.',
    })
    @ApiBearerAuth()
    @ApiBody({ type: CreateProductImageUploadUrlDto })
    @ApiStandardResponse(
        ProductImageUploadUrlResponseDto,
        'Signed upload URL created successfully',
        201,
    )
    @ApiStandardErrorResponse(
        400,
        'Invalid request',
        'Invalid content type or malformed request body',
    )
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.CREATED)
    async createImageUploadUrl(
        @Param('id') id: string,
        @Body() dto: CreateProductImageUploadUrlDto,
    ) {
        return this.createProductImageUploadUrlHandler.execute(
            new CreateProductImageUploadUrlCommand(id, dto.contentType),
        );
    }

    @Post(':id/images')
    @ApiOperation({
        summary: 'Finalize product image upload',
        description:
            'Persist metadata and attach an already uploaded S3 object key to the product.',
    })
    @ApiBearerAuth()
    @ApiBody({ type: FinalizeProductImageUploadDto })
    @ApiStandardResponse(ProductImageDto, 'Image finalized successfully', 201)
    @ApiStandardErrorResponse(
        400,
        'Invalid request',
        'Malformed S3 object key or invalid image metadata',
    )
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.CREATED)
    async finalizeImageUpload(
        @Param('id') id: string,
        @Body() dto: FinalizeProductImageUploadDto,
    ) {
        return this.finalizeProductImageUploadHandler.execute(
            new FinalizeProductImageUploadCommand(id, dto),
        );
    }

    @Post(':id/variants')
    @ApiOperation({
        summary: 'Add product variant',
        description:
            'Add a new variant (size, color, etc.) to an existing product',
    })
    @ApiBearerAuth()
    @ApiStandardResponse(
        ResponseVariantDto,
        'Variant created successfully',
        201,
    )
    @ApiStandardErrorResponse(
        400,
        'Invalid variant data',
        'Validation failed for variant creation',
    )
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.CREATED)
    async addVariant(
        @Param('id') id: string,
        @Body() variantDto: ProductVariantDto,
    ): Promise<ResponseVariantDto> {
        return this.createProductVariantHandler.execute(
            new CreateProductVariantCommand(id, variantDto),
        );
    }

    @Patch(':id')
    @ApiOperation({
        summary: 'Update product',
        description:
            'Update product information (name, description, status, etc.)',
    })
    @ApiBearerAuth()
    @ApiStandardResponse(ResponseProductDto, 'Product updated successfully')
    @ApiStandardErrorResponse(
        400,
        'Invalid update data',
        'Validation failed for product update',
    )
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.OK)
    async updateProduct(
        @Param('id') id: string,
        @Body() updateProductDto: UpdateProductDto,
    ): Promise<ResponseProductDto> {
        return this.updateProductHandler.execute(
            new UpdateProductCommand(id, updateProductDto),
        );
    }

    @Patch(':id/variants/:variantId')
    @ApiOperation({
        summary: 'Update product variant',
        description: 'Update a specific variant of a product',
    })
    @ApiBearerAuth()
    @ApiStandardResponse(ResponseVariantDto, 'Variant updated successfully')
    @ApiStandardErrorResponse(
        400,
        'Invalid update data',
        'Validation failed for variant update',
    )
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Variant not found',
        'Product or variant with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.OK)
    async updateVariant(
        @Param('id') id: string,
        @Param('variantId') variantId: string,
        @Body() variantDto: UpdateVariantDto,
    ): Promise<ResponseVariantDto> {
        return this.updateProductVariantHandler.execute(
            new UpdateProductVariantCommand(id, variantId, variantDto),
        );
    }

    @Patch(':id/images/:imageId')
    @ApiOperation({
        summary: 'Replace product image',
        description:
            'Replace an image mapping with a new finalized S3 object key and metadata.',
    })
    @ApiBearerAuth()
    @ApiBody({ type: FinalizeProductImageUploadDto })
    @ApiStandardResponse(ProductImageDto, 'Image replaced successfully')
    @ApiStandardErrorResponse(
        400,
        'Invalid request',
        'Malformed S3 object key or invalid image metadata',
    )
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Image not found',
        'Product or image with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.OK)
    async replaceImage(
        @Param('id') id: string,
        @Param('imageId') imageId: string,
        @Body() dto: FinalizeProductImageUploadDto,
    ) {
        return this.replaceProductImageHandler.execute(
            new ReplaceProductImageCommand(id, imageId, dto),
        );
    }

    @Delete(':id')
    @ApiOperation({
        summary: 'Delete product',
        description:
            'Delete a product and all its associated variants and images',
    })
    @ApiBearerAuth()
    @ApiStandardNoContentResponse('Product deleted successfully')
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Product not found',
        'Product with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.NO_CONTENT)
    async removeProduct(@Param('id') id: string) {
        await this.deleteProductHandler.execute(new DeleteProductCommand(id));
    }

    @Delete(':id/variants/:variantId')
    @ApiOperation({
        summary: 'Delete product variant',
        description: 'Delete a specific variant of a product',
    })
    @ApiBearerAuth()
    @ApiStandardNoContentResponse('Variant deleted successfully')
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Variant not found',
        'Product or variant with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.NO_CONTENT)
    async removeVariant(
        @Param('id') id: string,
        @Param('variantId') variantId: string,
    ) {
        await this.deleteProductVariantHandler.execute(
            new DeleteProductVariantCommand(id, variantId),
        );
    }

    @Delete(':id/images/:imageId')
    @ApiOperation({
        summary: 'Delete product image',
        description: 'Delete a specific image of a product',
    })
    @ApiBearerAuth()
    @ApiStandardNoContentResponse('Image deleted successfully')
    @ApiStandardErrorResponse(401, 'Unauthorized', 'Authentication required')
    @ApiStandardErrorResponse(403, 'Forbidden', 'Insufficient permissions')
    @ApiStandardErrorResponse(
        404,
        'Image not found',
        'Product or image with the given ID was not found',
    )
    @RequiredRoles('admin', 'employee')
    @UseGuards(JwtAccessTokenGuard, RolesGuard)
    @HttpCode(HttpStatus.NO_CONTENT)
    async removeImage(
        @Param('id') id: string,
        @Param('imageId') imageId: string,
    ) {
        await this.deleteProductImageHandler.execute(
            new DeleteProductImageCommand(id, imageId),
        );
    }
}
